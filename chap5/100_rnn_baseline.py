...
def train_1fold(output_dir, fold_id, config, num_workers, device, dryrun):
    print(f"Start training (fold_id = {fold_id}, config = {json.dumps(config)})")

    with simple_timer("Load training dataset"):
        num_train_samples = get_num_train_samples(dryrun)
        trn_df = pd.read_csv(TRAIN_CSV_PATH, nrows=num_train_samples, na_filter=False)
    ...
    with simple_timer("Predict OOF targets"):
        ...
        oof_df = pd.DataFrame({"id": trn_df.id, "is_duplicate": oof_predictions})

    torch.save(model.state_dict(), str(get_model_output_path(output_dir, fold_id)))
...
def predict_1fold(output_dir, fold_id, config, num_workers, device, dryrun):
    ...
    tst_df.to_csv(output_file, columns=["test_id", "is_duplicate"], index=False)
